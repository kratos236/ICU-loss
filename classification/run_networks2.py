...
class model():
    def __init__(self, config, data, test=False):
        ...
        if self.apply_pca:
            ...
            self.pca = IncrementalPCA(n_components=self.config['num_components'],
        ...
        for key, val in networks_defs.items():
            ...
            if scheduler_params['coslr']:
                ...
                self.model_scheduler_dict[key] = WarmupMultiStepLR(optimizer, scheduler_params['lr_step'],
                                                                   gamma=scheduler_params['lr_factor'],
                                                                   warmup_epochs=scheduler_params['warm_epoch'])
            else:
    ...
    def batch_forward(self, inputs, labels=None, feature_ext=False, phase='train'):
        ...
        if phase == 'train':
            self.logits, self.reg_loss = self.networks['classifier'](self.features, labels)
        else:
            self.logits, self.reg_loss = self.networks['classifier'](self.features, labels=None)
        # if self.apply_pca:
    ...
    def batch_loss(self, labels):
        ...
        if 'PerformanceLoss' in self.criterions.keys():
            ...
            self.loss += torch.sum(self.reg_loss)
        ...
        for epoch in range(1, end_epoch + 1):
            for key, model in self.networks.items():
                ...
                if self.config['networks'][key]['optim_params']['lr'] == 0.0:
                    print_write(['=====> module {} is set to eval due to 0.0 learning rate.'.format(key)],
                                self.log_file)
                    model.eval()
                else:
                    model.train()
